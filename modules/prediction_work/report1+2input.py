...
# IMPORTANT: For security, it is recommended to use environment variables instead of hardcoding.
API_KEY = "YOUR_API_KEY_HERE" 
# Replace the base URL with your actual API endpoint (e.g., local vLLM, Ollama, or OpenAI)
BASE_URL = "http://localhost:8000/v1" 
MODEL_NAME = "kimi-k2"  # Or any other model you need to use
...
# Process the first 10 reports: START_REPORT_INDEX = 0, END_REPORT_INDEX = 10
# Process all reports: START_REPORT_INDEX = 0, END_REPORT_INDEX = None
START_REPORT_INDEX = 0
END_REPORT_INDEX = 2500 # Set to an integer to limit the range, or set to None to process all files

# -- File Paths --
# NOTE: Please verify these paths are correct on your system.
...
OUTPUT_DIR = "./output/ckd_formal_results"  # Change output directory to avoid overwriting old results

# --- Main Prompt Template ---
# This is the final English prompt, now including a placeholder for previous reasoning logs.
PROMPT_TEMPLATE = """
You are a top-tier clinical AI reasoning expert, specializing in the intersection of nephrology and hepatology. Your core mission is to synthesize evidence from multiple independent sources, including two expert reports and a previous reasoning log, to generate an authoritative, de novo risk assessment for Chronic Kidney Disease (CKD) for a specific  participant with fatty liver disease.
...
def process_cluster(cluster_id, sample_ids, report_2_content, all_participant_data, all_previous_reasoning, api_client):
    ...
    for i, sample_id in enumerate(sample_ids):
        ...
        try:
            # ** MODIFICATION: Use streaming call to correctly separate thinking process and final response **
            stream = api_client.chat.completions.create(
                model=MODEL_NAME,
                messages=[{"role": "user", "content": final_prompt}],
                max_tokens=16384 * 4,
                temperature=0.2,
                stream=True  # CRITICAL: Set to True to enable streaming
            )
            ...
            for chunk in stream:
                if chunk.choices:
                    ...
                    if choice.delta and hasattr(choice.delta, "reasoning_content"):
                        if not thinking:
                            thinking = True
                        reasoning_parts.append(getattr(choice.delta, "reasoning_content", ""))
                    if choice.delta and choice.delta.content:
                        if thinking:
                            thinking = False
                        content_parts.append(choice.delta.content)

            reasoning = "".join(reasoning_parts) if reasoning_parts else "No reasoning process found in the response."
            content = "".join(content_parts) if content_parts else "No content found."
